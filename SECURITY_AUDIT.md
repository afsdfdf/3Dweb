...
| 项 | 理由 |
|----|------|
| 计数器（关注/点赞/评论）外部删除导致漂移 | 每次操作已按实际行数重算、常规路径自愈；全量周期重算需扫全表，成本与收益需先评估 |
| Meshy 并发上限在高速分发下存在窗口竞态 | 需 DB 级互斥或先占位再分发，改动较大且超额后果轻（提供商侧 429 会被处理） |
| 客户端 fetch 无统一超时与退避 | 前端体验优化，建议统一封装为 API client 后一次性解决 |
