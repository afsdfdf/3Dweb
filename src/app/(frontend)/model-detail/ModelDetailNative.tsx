...
import type { ModelDetailData, ModelDetailSideModel } from "./_lib/modelDetailData";
...
const fallbackData: ModelDetailData = {
  ...
  authorDescription: "Public creator model available for preview and reference.",
  authorName: "Xing Mu",
  commentsLabel: "0",
...
const getActiveModelFromSideModel = (item: ModelDetailSideModel): ActiveModel => ({
  id: Number(item.id),
...
export default function ModelDetailNative({ data = null, navUser = null }: ModelDetailNativeProps) {
  const detail = data ?? fallbackData;
  const [activeModel, setActiveModel] = useState<ActiveModel>(() => getInitialActiveModel(detail));
  const modelImages = activeModel.imageSrc ? [activeModel.imageSrc] : detail.previewImages.length > 0 ? detail.previewImages : fallbackData.previewImages;
  ...
  const goPrev = () => {
    setSlide((current) => (current - 1 + modelImages.length) % modelImages.length);
  };
      ...
      <div className={styles.scaleViewport}>
        ...
        <section className="detail-main">
          ...
          <section className="swiper-box" aria-label="Model preview">
            ...
            <button className="btn btn-prev" type="button" onClick={goPrev} aria-label="Previous model image" />
            <button className="btn btn-next" type="button" onClick={goNext} aria-label="Next model image" />
          </section>

          <div className="tips-box" style={{ left: "50%", marginLeft: 200, top: "80%" }}>
            <div className="dot">
              <i />
            </div>
          ...
          {showDownloadConfirm ? (
            <ModelDownloadConfirmation
              ...
              onConfirm={() => {
                if (detail.id) {
                  window.location.href = `/api/platform/mock/models/${activeModel.id}/download?format=glb`;
                }
              }}
        ...
        <section className="detail-right">
          <ButtonBoxFrame className="detail-right-frame" contentClassName="detail-right-frame-content">
            ...
            <div className="info">
              <div className="item">
                <img src={asset("detail-side-icon-1-1.png")} alt="" className="uc-icon24" decoding="async" />
                {detail.viewLabel}
              </div>
              <div className="item">
                <img src={asset("detail-side-icon-1-2.png")} alt="" className="uc-icon24" decoding="async" />
                {detail.likesLabel}
              </div>
            ...
            <ul className="list">
              <li>
                <div className="img">
                  <a href={`/model-detail?id=${activeModel.id}`} onClick={(event) => event.preventDefault()}>
                    <img
                      src={activeModel.imageSrc || asset("detail-side-img.png")}
                      alt={activeModel.title}
                      decoding="async"
                      loading="eager"
                    />
                ...
                <div className="right">
                  ...
                  <div className="tags">
                    {activeModel.tags.map((tag) => (
                      <a href="#" className="uc-tag" key={tag} title={`# ${tag}`}># {tag}</a>
                    ))}
                  </div>
                </div>
              </li>
            ...
            <div className="form-box">
              <textarea className="uc-textarea" placeholder="Please enter your comment." rows={10} />
              <div className="right">
                <a href="#" className="uc-btn">COMMENT</a>
                <div className="number">0/500</div>
              </div>
            </div>
            <div className="total">{detail.commentsLabel} Comments</div>
            <div className="model-scroll">
              <div className="side-title">{detail.isOwnedByCurrentUser ? "MY MODEL" : "CREATOR MODELS"}</div>
              <ul className="list2">
                {relatedModels
                  ? relatedModels.map((item) => (
                      <li
                        key={item.id}
                      >
                        <ButtonBoxFrame className="side-model-card-frame" contentClassName="side-model-card-content">
                          <a
                            aria-current={item.id === activeModelId ? "page" : undefined}
                            className="item"
                            href={item.href}
                            onClick={(event) => {
                          ...
                          >
                            <img src={item.imageSrc} alt={item.title} decoding="async" loading="lazy" />
                          </a>
                        </ButtonBoxFrame>
                      </li>
                    ))
                      ...
                      <li key={item}>
                        <ButtonBoxFrame className="side-model-card-frame" contentClassName="side-model-card-content">
                          <a href="#" className="item">
                            <img src={asset("detail-side-img-1.png")} alt="" decoding="async" loading="lazy" />
                          </a>
                        </ButtonBoxFrame>
                      </li>
                    ))}
        ...
        <div className="search-box">
          <input type="text" className="uc-input" placeholder="Please enter keywords" />
          <img src={asset("icon-search-white.png")} className="icon" alt="" decoding="async" />
          <a href="#" className="uc-btn">Search</a>
        </div>

        <section className="detail-bottom">
          <div className="left">
            <div className="points">
              <img src={asset("detail-bottom-icon-1.png")} className="icon" alt="" decoding="async" />
              <div className="hd">
            ...
            <a href="#" className="btn">
              <img src={asset("detail-bottom-icon-2.png")} alt="" decoding="async" />
              {detail.viewLabel}
            </a>
            <a href="#" className="btn">
              <img src={asset("detail-bottom-icon-3.png")} alt="" decoding="async" />
              {detail.likesLabel}
            </a>
            <a href="#" className="btn">
              <img src={asset("detail-bottom-icon-4.png")} alt="" decoding="async" />
          ...
          <div className="center">
            ...
            <div className="btn2-slot">
              <SourcePurpleMediumButton
                label="PRINT NOW"
                onClick={() => {
                  window.location.href = activeModel.id ? `/workbench?reference=${activeModel.id}` : "/workbench";
                }}
                style={detailBottomActionButtonStyle}
              />
              <img
          ...
          <div className="right">
            <div className="hd">
              <img src={asset("detail-bottom-icon-5.png")} alt="" decoding="async" />
            </div>
            <div className="bd">
              <a href={activeModel.id ? `/workbench?reference=${activeModel.id}` : "#"} className="item">Use As Reference</a>
              <a href="#" className="item">Favorite Model</a>
              <a href={activeModel.id ? `/showcase/${activeModel.id}` : "#"} className="item">Open Showcase</a>
            </div>
          </div>
        </section>
          </section>
