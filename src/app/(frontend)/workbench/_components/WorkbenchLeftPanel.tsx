...
type PrimaryMode = 'image3d' | 'imagegen' | 'text3d'
...
export function WorkbenchLeftPanel() {
  const [primaryMode, setPrimaryMode] = useState<PrimaryMode>('image3d')
  ...
  const sectionTitle = useMemo(() => 'Image', [])
  ...
  return (
    <WorkbenchSideContainer>
      <div className={styles.leftPanel}>
        ...
        <div className={`${styles.secondaryTabs} ${!isImageGen ? styles.secondaryTabsHidden : ''}`}>
            <button
        ...
        <section className={styles.fieldGroup}>
          ...
          <div className={styles.inputWrap}>
            <button className={styles.selectButton} onClick={() => setShowTitleMenu((value) => !value)} type="button">
        ...
        <section className={styles.fieldGroup}>
          <div className={styles.multiViewHeader}>
            ...
            <div className={styles.multiViewToggleRow}>
              <span className={styles.toggleText}>{multiView ? 'on' : 'off'}</span>
              <button className={styles.toggleTrack} onClick={() => setMultiView((value) => !value)} type="button">
                <span className={`${styles.toggleThumb} ${!multiView ? styles.toggleThumbOff : ''}`} />
              </button>
            </div>
          </div>

          <div className={`${styles.multiViewBox} ${!multiView ? styles.multiViewBoxHidden : ''}`}>
              {Array.from({ length: 3 }).map((_, index) => (
                <button className={styles.multiViewCard} key={`multi-${index + 1}`} type="button">
                  <span className={styles.addImageIcon}>
                    <Plus size={16} />
                  </span>
                  <span className={styles.addImageLabel}>Add Image</span>
        ...
        <section className={styles.fieldGroup}>
          ...
          <div className={styles.inputWrap}>
            <button className={styles.selectButton} onClick={() => setShowLicenseMenu((value) => !value)} type="button">
              <span>{license}</span>
              <ChevronDown size={16} />
            </button>
