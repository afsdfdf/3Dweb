...
import { Box, ChevronLeft, Download, Eye, Heart, MoreHorizontal, Printer, Search, ShieldCheck, Sparkles, Star, Users } from 'lucide-react'
...
function formatDimensions(model: WorkbenchModel) {
  ...
  if (![width, height, depth].some((value) => typeof value === 'number')) return 'Not specified'
      ...
      >
        <article className="flex h-full flex-col gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-[#f3ecdf]">{model.title}</div>
...
function GalleryCard({
  ...
  return (
    <Link className="group block h-[160px] w-[92px]" href={`/workbench/models/${model.id}`}>
      <LineFrame
      ...
      >
        <div className="h-full overflow-hidden rounded-[2px] border border-[#2f2c33] bg-[linear-gradient(180deg,#18191d_0%,#0b0b0d_100%)]">
          {model.previewURL ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img alt={model.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" src={model.previewURL} />
          ) : (
...
export default async function WorkbenchModelDetailPage({
  ...
  const selectedModel = previewMode ? models[0] ?? buildPreviewModel() : models.find((item) => String(item.id) === String(id))
  ...
  const sidebarModels = models
    .filter((item) => item.id !== selectedModel.id && (selectedModel.ownerId ? item.ownerId === selectedModel.ownerId : true))
    .slice(0, 6)
  const fallbackModels = models.filter((item) => item.id !== selectedModel.id).slice(0, 6)
  const galleryModels = sidebarModels.length > 0 ? sidebarModels : fallbackModels
  const downloadCredits = formatDownloadCredits(selectedModel)
  const ownerProfile = selectedModel.ownerProfile
  const heroBanner = ownerProfile?.backgroundUrl || selectedModel.previewURL || '/ui/frames/hello.png'
  const downloadFormat = selectedModel.formats.find((item) => item.format.toLowerCase() === 'glb')?.format || selectedModel.formats[0]?.format || 'glb'
  const creatorBio =
    ownerProfile?.bio ||
  ...
  if (previewMode) {
    const [locale, navUser] = await Promise.all([getCurrentLocale(), getCurrentNavUser()])
    ...
    return (
      <div className="grid h-screen w-screen place-items-center overflow-hidden bg-[#181818]" style={previewViewportStyle}>
        <div
          ...
          <div className="grid min-h-0 grid-rows-[minmax(0,1fr)_92px] gap-3">
            <div className="relative min-h-0 overflow-hidden rounded-[4px] bg-[radial-gradient(circle_at_18%_78%,rgba(106,124,144,0.34),transparent_36%),radial-gradient(circle_at_78%_18%,rgba(121,78,34,0.26),transparent_28%),linear-gradient(180deg,#060607_0%,#121418_58%,#26282d_100%)] shadow-[0_30px_56px_rgba(0,0,0,0.48)]">
              ...
              <div className="absolute inset-0">
                <ModelViewer className="h-full w-full" label={selectedModel.title} src={selectedModel.viewerURL} />
              </div>
              ...
              <div className="absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-6 px-8 pt-6">
                <div className="max-w-[520px]">
                  ...
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img alt="" aria-hidden="true" className="mt-5 h-[2px] w-full max-w-[380px] object-cover opacity-55" src={TOP_DIVIDER_SRC} />
                  <div className="mt-4 text-[11px] uppercase tracking-[0.22em] text-[#8b8590]">Model Name</div>
                  <h1 className="mt-3 text-[58px] font-black leading-[0.95] tracking-tight text-white">{selectedModel.title}</h1>
                ...
                <div className="grid min-w-[208px] gap-3 rounded-[4px] border border-[#35333a] bg-[#09090c]/84 px-4 py-5 shadow-[0_18px_36px_rgba(0,0,0,0.35)]">
                  {[
                    ...
                    <div className="flex items-center justify-between gap-4" key={item.label}>
                      <span className="text-[10px] uppercase tracking-[0.16em] text-[#7f7983]">{item.label}</span>
                      <span className="text-sm font-medium text-[#f1ebdf]">{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img alt="" className="h-16 w-16" src="/ui/frames/dark-3d-arrow-left.png" />
              </button>
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img alt="" className="h-16 w-16" src="/ui/frames/dark-3d-arrow-right.png" />
              </button>

              <div className="absolute bottom-5 left-8 z-10 w-[312px]">
                <form action="/workbench" className="flex h-14 items-center overflow-hidden rounded-[4px] border border-[#5b585f] bg-[#17181c]/92 shadow-[0_16px_34px_rgba(0,0,0,0.38)]">
                  ...
                  <FrameButton height={42} size="compact" type="submit" variant="slate" width={98}>
                    Search
                  </FrameButton>
                </form>
              </div>
              ...
              <div className="absolute bottom-14 right-[12%] z-10 hidden w-[380px] lg:block">
                ...
                >
                  <div>
                    <div className="text-center text-[13px] font-semibold tracking-[0.02em] text-[#f0d08c]">Model Download Confirmation</div>
                    <p className="mt-3 text-center text-sm leading-6 text-[#beb6aa]">
                      Downloading the current model will cost 1 to 2 points.
                    </p>
                    <div className="mt-6 grid grid-cols-2 gap-3">
                      <FrameButton height={52} variant="slate">Cancel</FrameButton>
            ...
            >
              <div className="grid h-full grid-cols-[320px_1fr_auto] items-center gap-5">
                ...
                <div className="flex items-center gap-4">
                  <MetricTile icon={<Eye className="size-4" />} label="Views" value={formatCompactNumber(selectedModel.viewCount)} />
                  <MetricTile icon={<Heart className="size-4" />} label="Likes" value={formatCompactNumber(selectedModel.likesCount)} />
                  <MetricTile icon={<Star className="size-4" />} label="Favorites" value={formatCompactNumber(selectedModel.favoritesCount)} />
                </div>
              ...
              <div className="grid min-h-0 grid-rows-[90px_116px_90px_74px_17px] gap-[10px] overflow-hidden">
                <div className="mx-auto w-[320px] overflow-hidden rounded-[4px] border border-[#2e2b31] bg-[#111216]">
                  <div className="relative h-full overflow-hidden">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img alt={`${selectedModel.ownerName} banner`} className="h-full w-full object-cover" src={heroBanner} />
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4">
                      <div className="relative inline-flex min-h-[34px] min-w-[188px] items-center justify-center px-7">
                        ...
                        <span className="relative text-[11px] uppercase tracking-[0.18em] text-[#f0d08c]">
                          {selectedModel.isOwnedByCurrentUser ? 'My Model' : 'Creator Archive'}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative shrink-0 rounded-full border-[5px] border-[#d69d57] bg-[#2b1612] p-1 shadow-[0_0_0_1px_rgba(255,255,255,0.08)]">
                        <div className="flex size-[72px] items-center justify-center overflow-hidden rounded-full bg-[linear-gradient(180deg,#fff3e0_0%,#d0c4c7_100%)] text-3xl font-black text-[#2a272d]">
                          {(ownerProfile?.displayName || selectedModel.ownerName).slice(0, 1).toUpperCase()}
                        </div>
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <div className="truncate text-[16px] font-semibold text-[#f3ecdf]">{ownerProfile?.displayName || selectedModel.ownerName}</div>
                          <span className="rounded-[4px] border border-[#4a454c] bg-[#17171a] px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-[#f0eadc]">
                            {selectedModel.isOwnedByCurrentUser ? 'Owner' : 'Creator'}
                          </span>
                        </div>
                        <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#a9a29a]">{creatorBio}</p>
                      </div>
                    </div>

                    <div className="mt-auto flex items-center justify-between border-t border-[#28252b] pt-4">
                      <SidebarMetric icon={<Users className="size-4" />} value={formatCompactNumber(ownerProfile?.followersCount || 0)} />
                      <SidebarMetric icon={<Box className="size-4" />} value={`${ownerProfile?.followingCount || 23}`} />
                      <div className="text-sm text-[#a8a1ab]">{formatWorkbenchDate(selectedModel.updatedAt || selectedModel.createdAt).split(' ')[0]}</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="overflow-hidden rounded-[4px] border border-[#312e35] bg-[#18191d]">
                      {selectedModel.previewURL ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img alt={selectedModel.title} className="size-16 object-cover" src={selectedModel.previewURL} />
                      ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] uppercase tracking-[0.16em] text-[#8c8690]">Image Input</div>
                      <div className="mt-2 text-sm text-[#9e97a3]">{formatWorkbenchDate(selectedModel.updatedAt || selectedModel.createdAt)}</div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {selectedModel.tags.slice(0, 3).map((tag) => (
                          <span className="rounded-[4px] border border-[#3a373f] bg-[#151519] px-2 py-1 text-[11px] text-[#e7dece]" key={tag}>
                            #{tag}
                          </span>
                        ))}
                      </div>
              ...
              <div className="grid min-h-0 grid-rows-[34px_1fr] gap-[10px] overflow-hidden pt-[10px]">
                <div className="flex items-center justify-center">
                  <div className="relative inline-flex h-[34px] w-[256px] items-center justify-center px-7">
                    ...
                    <img alt="" aria-hidden="true" className="absolute inset-0 h-full w-full object-fill" src={ORNAMENT_STRIP_SRC} />
                    <span className="relative text-[11px] uppercase tracking-[0.18em] text-[#f0d08c]">
                      {selectedModel.isOwnedByCurrentUser ? 'My Model' : 'Creator Library'}
                    </span>
                  </div>
                </div>
