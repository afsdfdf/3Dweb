...
export async function getWorkbenchModels(user: Awaited<ReturnType<typeof getCurrentUser>>): Promise<WorkbenchModel[]> {
  ...
  return Promise.all(
    result.docs.map(async (model: any) => {
      ...
      const ownerName =
        normalizeText(owner?.displayName) || normalizeText(owner?.fullName) || normalizeText(owner?.email) || 'Unknown creator'
      const formats = Array.isArray(model.formats)
        ? model.formats
            .map((item: any) => {
              ...
              return {
                downloadCredits: typeof item?.downloadCredits === 'number' ? item.downloadCredits : null,
      ...
      return {
        ...
        dimensions:
          isRecord(model.dimensions)
            ? {
                depthMm: typeof model.dimensions.depthMm === 'number' ? model.dimensions.depthMm : null,
                heightMm: typeof model.dimensions.heightMm === 'number' ? model.dimensions.heightMm : null,
                widthMm: typeof model.dimensions.widthMm === 'number' ? model.dimensions.widthMm : null,
              }
            : null,
        ...
        ownerProfile: owner
          ? {
              ...
              profileVisibility: normalizeText(owner.profileVisibility) || 'private',
            }
          : null,
        previewURL,
        printReady: Boolean(model.printReady),
        sourceTaskCode: typeof sourceTask?.taskCode === 'string' ? sourceTask.taskCode : null,
        status: typeof model.status === 'string' ? model.status : 'draft',
        tags: Array.isArray(model.tags) ? model.tags.map((tag: any) => String(tag?.label || '').trim()).filter(Boolean) : [],
        title: typeof model.title === 'string' && model.title.trim() ? model.title : `Model ${model.id}`,
        updatedAt: typeof model.updatedAt === 'string' ? model.updatedAt : null,
        viewCount: Number(model.viewCount || 0),
        viewerURL,
        visibility: typeof model.visibility === 'string' ? model.visibility : 'private',
      }
    }),
  )
}
