...
- Footer link and logo URL safety lives in `src/app/(frontend)/_components/shell/footerSafety.ts`. Keep protocol-relative URLs, control-character URLs, unsafe protocols, and private Supabase object URLs blocked there, and keep the Site Settings `footer.linkGroups` `maxRows` aligned with the public footer display cap.
