...
- The `/blog` top banner image is editable through `formal-pages.blogPage.heroImage` in the "Blog hero banner and CTAs" admin group. The selected media must be guest-readable (`publicAccess` enabled or preview purpose) because public rendering resolves it through `getGuestReadableBlogImageURL`.
...
- Public blog post reads must prefer the current frontend locale and keep Payload `fallbackLocale: false` for article records. If the current locale has no public title/content matches, `blogData.ts` may explicitly query the other supported locale with the same `_status`, `isVisible`, `publishedAt`, title, and content guards so single-locale operator content does not leave `/blog` blank. Do not re-enable Payload localized fallback or pin `posts` queries to English.
