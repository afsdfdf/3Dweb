...
- `/account` now owns the formal personal center route and renders the former `personal-center-test` UI with server-side current-user data for profile, credits, transactions, tasks, models, and orders. `/personal-center-test` redirects to `/account`; do not revive parallel personal-center routes. Profile and password edits use the existing `/api/account/profile` and `/api/account/password` endpoints without changing backend schemas.
...
- `requireUser()` accepts an optional redirect target and defaults to `/dashboard/tasks` for dashboard-era callers. `/account` must call `requireUser("/account")` so the login modal returns the user to the personal center instead of the old task dashboard after authentication.
