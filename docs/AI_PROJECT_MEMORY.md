...
- Public blog post reads must follow the current frontend locale and use `fallbackLocale: false` for article records. Do not pin `posts` queries to English or let localized fallback make a post appear in a locale where its localized title is absent.
