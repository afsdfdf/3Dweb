...
## 2026-06-11 Frontend Route Loading UX

- Do not add frontend `loading.tsx` route fallbacks for the public frontend unless explicitly requested. Most public pages still read cookies/session or Payload-backed content, so Vercel serves them dynamically and client route transitions can wait several seconds. Route loading fallbacks turn that wait into a visible spinner or skeleton during normal page navigation; the desired behavior is to stay on the current page until the destination is ready. Keep `tests/frontendLoadingUX.test.ts` guarding against reintroducing these page-switch loading animations.
...
- The cart modal frame uses the existing `ButtonBoxFrame` asset family with a desktop design size of `1160px x 760px`. Cart-specific PC slice assets live in `public/ui-lab/cart-modal/` and drive the logo, checkbox states, close icon, price chip, subtotal strip, checkout arrow, and quantity plus/minus icons.
- `/checkout` is a front-end placeholder route for selected cart items. It is not a backend payment session and does not create Payload `print-orders` or Stripe checkout sessions.
- `ModelDetailNative` adds `ADD TO CART` by snapshotting the active model into local cart storage with the current front-end print quote values. Replace this with backend price snapshots before adding real multi-item checkout.
- The shared `TopNavigation` cart glyph now links to `/cart`; order history remains `/account?section=orders`.

## 2026-06-04 Account Profile UI

- `/account` now presents a visual account frame with restored personal-center secondary tabs: `PROFILE`, `POINTS HISTORY`, `ORDERS`, `MODEL LIBRARY`, and `GENERATION TASKS`. The outer frame uses the same full 15-piece `BorderComboFrame2` component that the homepage third `Inspiration` frame uses, not the simplified `BorderComboFrame2Variant`. The account frame should match the homepage third frame metrics: `--upper-height: calc(112px - var(--top-height) - (var(--middle-height) / 2))`, content padding `0 var(--right-width) var(--bottom-height) var(--left-width)`, a 58px top-navigation logo column, a 240px `banner-label-bg.png` title plate, and a divider with the same 26px top offset. `accountChrome` owns only this homepage-style title row; `accountBody` owns the secondary tab strip plus the unified black scrollable work area for profile, points history, and record tables. Account secondary tabs and small edit controls should render as flat dark, thin-border controls to match the Lanhu account reference; keep the dark `formal-components/assets/small-button-pair/button-dark-small-*` and `/ui/buttons/framed/slate-*` paths only as asset-family references, not as glossy visible fills. Recharge keeps `/ui/buttons/framed/gold-*`. Do not reintroduce the old `accountFrame::before` overlay or `[class*="middle11"]` overrides; those were workarounds for the wrong Variant frame. At desktop heights below `980px`, compress only the account content rhythm, balance strip, and table row height so the frame stays one-screen instead of clipping the bottom; do not compress the homepage-style title row. The default `profile` tab owns avatar upload, background upload, account-name editing, read-only registration email display, and password editing. Only one profile field can be edited or uploaded at a time.
- The points tab uses `/account?section=points-history`, shows the current credit balance plus credit transaction rows, and keeps the old `billing` query param mapped to points history. Legacy `overview` and `settings` section params map back to profile for compatibility. The balance strip uses the account-specific wide `/ui-lab/account-center/account-balance-plate-wide@2x.png` asset as a single background; that derived asset preserves the coin and right chevrons while stretching only the center strip so the `BALANCE` text and `RECHARGE` button do not crowd each other. Do not rebuild it with CSS gradients, use the compact 340px strip, or layer a separate coin image over it. Points History pagination should not render numeric page buttons; keep only previous/next arrow buttons, a plain text current-page indicator, and a plain text page-size label.
- The account route fetches profile summary, current credits, credit transactions, user-owned models, generation tasks, and print orders. Credit transaction list loading stays lightweight outside the points-history tab by using `depth: 0`, explicit `select` fields, and the summary limit; models/tasks/orders use the existing current-user helpers with explicit owner/user `where` clauses. The `MODEL LIBRARY` type column owns the Public/Hidden toggle and must call `/api/account/models/:modelId/visibility` with `public` or `private`, never direct Payload collection mutation from the browser. The route uses `getMarketingSiteSettings()` for header navigation chrome.

## 2026-06-09 Credit Top-Up Redemption UI

- One-time credit purchases stay backed by existing `credit-products` records with `productType = credit-topup` and the existing `/api/billing/credits/checkout` endpoint. Do not add arbitrary user-entered credit amounts unless a separate billing design adds server-side pricing, limits, and webhook validation for custom quantities.
- `src/lib/getCreditTopupProducts.ts` is the shared server helper for active credit top-up packs; `src/lib/creditTopupProducts.ts` owns client-safe normalization, filtering, sorting, and fixed-pack previous/next selection.
- `CreditTopupRedemptionDialog` is the shared point redemption modal. It uses `ButtonBoxFrame`, the shared coin asset, fixed credit-pack `- / +` selection, and an orange `BUY` action that posts the selected `productId` to the existing credit checkout endpoint. Account `RECHARGE`, top navigation user-menu `Point Redemption`, and the pricing page Credit Packs quick entry should open this same component.
- `/pricing` keeps subscription plans as the fixed primary subscription surface and adds a separate `Credit Packs` quick entry for one-time top-ups. The quick entry should not replace subscription checkout or mutate subscription plan definitions.
...
- Footer brand content is also owned by `site-settings.footer`: `brandLogo`, `brandLogoAlt`, and `brandSummary`. The admin UI registers `/components/admin/FooterPreview` for a live footer preview, and `FooterBar` only renders uploaded footer logos when the selected media is guest-readable (`publicAccess = true` or `purpose = preview`), otherwise it falls back to the default wordmark.
- Footer social/media links are also owned by `site-settings.footer.socialLinks` and persisted in `site_settings_footer_social_links`. Rows have `platform`, `label`, `href`, and `enabled`; disabled rows stay configured in admin but are hidden from the public footer. Footer link, logo, and social URL safety lives in `src/app/(frontend)/_components/shell/footerSafety.ts`. Keep protocol-relative URLs, control-character URLs, unsafe protocols, and private Supabase object URLs blocked there, and keep the Site Settings `footer.linkGroups` / `footer.socialLinks` `maxRows` aligned with the public footer display caps.
...
- Formal public navigation should resolve through `site-settings.headerNav` using `resolvePublicNavigationItems`. The adapter preserves known IDs such as `HOME`, `WORKBENCH`, `SHOWCASE`, `ASSETS`, `BLOG`, and `ABOUT` while allowing backend-managed labels and order to drive `TopNavigation` and mobile formal headers. Avoid new production uses of `migrationTestNavItems` on CMS-backed formal pages.
...
- Public pricing cards use minimum heights instead of fixed clipped content boxes so backend-managed plan features can grow without silently hiding text. The top-navigation cart glyph links to `/cart`, while order history remains `/account?section=orders`.
...
- Top-navigation subscription promotion copy is owned by `site-settings.navigationPromotion`. The global stores `enabled`, `eyebrow`, `offerText`, `buttonLabel`, and `buttonAriaLabel`; public pages should pass that merged marketing setting into `TopNavigation` instead of hardcoding promo text in route components.
- The top navigation shows the subscription promotion to anonymous visitors and signed-in users without an active subscription. `getCurrentNavUser()` derives `hasActiveSubscription` from `billing-subscriptions` using access-controlled Local API reads (`user` plus `overrideAccess: false`) so UI state does not bypass subscription access rules.
- `GET /api/billing/subscriptions/plans` is the public-safe plan summary endpoint used by the top-navigation subscription dialog. It returns editable display price/credits/copy but intentionally omits Stripe `lookupKey`; authenticated checkout still goes through `POST /api/billing/subscriptions/checkout`, where the server accepts only fixed plan keys and resolves Stripe Price IDs internally. Anonymous CTA clicks should open the shared login modal rather than creating checkout sessions.
- Subscription checkout now auto-aligns Stripe Prices with Payload-managed plan pricing. `ensureStripePlanPrice()` checks the current Stripe Price behind each fixed lookup key; when the active Price amount no longer matches `site-settings.subscriptionPlans.*.monthlyPrice`, it creates a replacement monthly Price, uses `transfer_lookup_key` to move the lookup key, and archives the old active Price. This preserves historical lookup key names while keeping new checkout sessions in sync with Payload pricing; existing subscriptions keep their existing Stripe Price until Stripe subscription migration rules are explicitly applied. The legacy Supabase billing compatibility path delegates to this same helper.
...
- Subscription plans support both `monthly` and `yearly` billing cycles. Payload `site-settings.subscriptionPlans.*` owns `monthlyPrice` and `yearlyPrice`; Stripe lookup keys remain code-owned (`miniforge_<plan>_monthly` and `miniforge_<plan>_yearly`) so operators change prices in Payload rather than editing Stripe lookup keys. `ensureStripePlanPrice(plan, cycle)` rotates the active Stripe Price for the selected cycle and writes `billingCycle`, `creditsPerMonth`, and `creditsPerPeriod` metadata. Yearly subscriptions grant `creditsPerMonth * 12` for the annual billing period until a monthly scheduled grant design exists.
- `billing-checkouts` is the internal pending checkout lock collection for subscription checkout. `createSubscriptionCheckout()` creates one open lock per user before calling Stripe, passes a Stripe idempotency key, stores the returned session URL, and reuses the open URL on repeat requests instead of creating duplicate Stripe sessions. Checkout sync or webhook completion marks the record `completed` and clears `openLockKey`; stale or failed attempts become `expired` or `failed`.
- Billing-facing reads fail closed. `GET /api/billing/subscriptions/plans`, subscription checkout, subscription portal, and subscription sync use strict Site Settings reads; if payment provider or plan settings cannot be read, plans return unavailable/disabled and checkout must not silently fall back to default display prices.

### 2026-06-14

- Homepage hero header background is editable through `homepage-content.hero.headerBackground`. The public home hero reads that media first and falls back to `/home-test-assets/images/home-hero-header-background.jpg`. Operators should choose media that is guest-readable through `purpose = preview` or explicit `publicAccess = true`, matching the homepage public media boundary.
