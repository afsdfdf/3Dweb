...
- Source language cleanup added `scripts/audit-source-language.mjs` and `npm`/package script `audit:source-language`. The audit fails on Chinese characters outside explicit `src/i18n/**` resources and on common mojibake markers, while excluding generated files, migrations, test routes, `GenerateForm.tsx`, and the known legacy personal-center component tree.
...
- Password recovery uses the auth modal `forgot-success` state for the "Check Your Email" confirmation. The reset-password page should submit to `/api/account/auth/reset-password`, not Payload's raw `/api/users/reset-password`, so the account auth endpoint can set the session cookie and return the user to `/account` after a successful reset.
